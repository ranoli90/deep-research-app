"""Synthetic emulator control only. Refuses physical devices; redacts unrelated app nodes."""
...
if adb("shell", "getprop", "ro.kernel.qemu").strip() != b"1":
    raise SystemExit("Expected isolated emulator; refusing device interaction")
mode = sys.argv[1]
if mode == "text":
    text = sys.argv[2]
    ...
    path = "/sdcard/deep-v6-emulator-window.xml"
    try:
        adb("shell", "uiautomator", "dump", path)
        root = ET.fromstring(adb("shell", "cat", path))
        allowed = {"app.deepresearch.mobile", "com.google.android.documentsui", "com.android.documentsui", "com.android.intentresolver", "android", "com.android.permissioncontroller"}
