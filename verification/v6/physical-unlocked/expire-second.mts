/** Exact synthetic second account deletion to exercise expired-session native recovery. */
...
const id='ec0fd00c-7020-467c-80b2-477fd93f89e5';
try {
 const {rows}=await pool.query('SELECT id,created_at,deleted_at FROM accounts WHERE id=$1',[id]);
 if(rows.length!==1||new Date(rows[0].created_at).toISOString()!=='2026-09-18T13:20:34.629Z')throw Error('Synthetic account identity mismatch');
