...
try{
 ...
 const rows=(await pool.query(`SELECT CASE WHEN provider_key_scope=$1 THEN 'current_project_key' ELSE 'unattributed' END AS attribution,
 COUNT(*)::int AS intents,COALESCE(SUM(confirmed_micro),0)::text AS confirmed_micro,
 COUNT(*) FILTER(WHERE confirmed_micro IS NULL)::int AS unconfirmed_intents,
 COALESCE(SUM(reserved_max_micro) FILTER(WHERE confirmed_micro IS NULL),0)::text AS held_micro
 FROM provider_intents WHERE route LIKE 'openrouter:%' AND(provider_key_scope=$1 OR provider_key_scope IS NULL) GROUP BY attribution`,[keyScope])).rows;
...
writeFileSync(new URL('./PROJECT_LEGACY_EXPOSURE.json',import.meta.url),JSON.stringify(result,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(result,null,2));process.exitCode=Number(result.exitCode);
