...
(async () => {
  ...
  const upload = await request('/v1/attachments/bytes', 'POST', bytes, { 'content-type': 'application/octet-stream', 'x-document-mime': 'application/pdf', 'x-file-name': 'SourceDeletionControl.pdf' });
  const question = 'What does the Ardent field note say about underwater recording?';
  const created = await request('/v1/runs', 'POST', JSON.stringify({ question, routeMode: 'controlled-research', attachmentIds: [upload.attachmentId], consentPolicyVersion: consent.policyVersion }), { 'idempotency-key': crypto.randomUUID() });
  let run;
  for (let i = 0; i < 180; i++) { run = await request(`/v1/runs/${created.runId}`); if (run.lifecycle === 'terminal') break; await delay(500); }
  if (!run?.reportId || run.outcome !== 'completed') throw new Error('Synthetic parent did not produce its expected checked report');
  const report = await request(`/v1/reports/${run.reportId}`);
  ...
  try {
    const expression = `(()=>{const find=n=>{const p=Array.from(__r.getModules().entries()).find(x=>x[1].verboseName===n);if(!p)throw Error('Project module absent');return __r(p[0]);};const storage=find('src/native-session.ts').sessionStorage;globalThis.__deepDraftSetup=null;const session=${JSON.stringify({ accountId: session.accountId, token })};const state=Object.assign(find('src/state.ts').emptyState(),${JSON.stringify({ signedIn: true, consentGranted: true, routeMode: 'controlled-research', status: 'completed', draft: question, run, report })});storage.activate(session).then(()=>storage.persist({token:session.token,state})).then(()=>storage.flush()).then(()=>{globalThis.__deepDraftSetup={ready:true};},()=>{globalThis.__deepDraftSetup={ready:false};});return 'started';})()`;
    ...
    console.log(JSON.stringify({ evidenceClass: 'programmatic_owned_API_parent_setup_for_native_source_deletion', accountId: session.accountId, runId: run.runId, reportId: report.reportId, inputBytes: bytes.length, inputSha256: crypto.createHash('sha256').update(bytes).digest('hex'), actualParser: true, modelTransport: 'fabricated', paidCalls: 0, nativeSignInAndUploadRepeated: false }, null, 2));
  } finally { ws.close(); }
