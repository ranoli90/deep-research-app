...
if grant['budgetMicro']!=400000 or grant['taskIds']!=['MC-D01'] or grant['sourceMode']!='frozen_supplied_document':raise SystemExit('Unexpected registered scope')
...
receipt={'command':command,'commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'startedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'scope':'Real OpenRouter model, frozen public SQLite bytes, production API/worker/parser/checker/writer; $0.40 maximum new allowance','environment':{'node':subprocess.check_output(['node','--version'],text=True).strip(),'pnpm':subprocess.check_output(['pnpm','--version'],text=True).strip(),'platform':'Linux local PostgreSQL'}}
