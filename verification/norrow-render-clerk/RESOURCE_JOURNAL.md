...
| 2026-09-20 continuation readback | Authenticated `GET /v1/projects/{id}`, `GET /v1/environments/{id}`, `GET /v1/postgres/{id}`, and `GET /v1/postgres/{id}/recovery` | Project and both environments returned `200`; each database remains `available` with its recorded plan/version/empty database `ipAllowList`. Both recovery endpoints now return `recoveryStatus: AVAILABLE`. Environment network isolation remains enabled; its tier-default ingress list remains `0.0.0.0/0`. | READBACK_VERIFIED. `AVAILABLE` establishes provider recovery availability only. It does not establish a PITR window, backup contents, a restore, failover, application recovery, or any deployed-service readiness. |

The current workspace cannot configure restrictive environment-level IP allowlists. The supported environment controls to preserve are protection and cross-environment network isolation. No creation response is accepted as final until it is re-read by resource ID and each supported required property is present.
...
- Both project environments are protected and network-isolated. Their tier-limited environment-level allowlist remains the provider default `0.0.0.0/0`; that scope is neither the database external-access control nor evidence of an attached service's route policy.
- These are separate controls: (1) the private databases deny external connections through their empty database allowlists; (2) network isolation prevents cross-environment private-network reachability; (3) a future staging operator path must use the supported deployment/operator and service-access controls selected for that path; and (4) an intentionally public application API is public at transport ingress but must default-deny work-creating routes through its server-owned authentication, admission, budget, consent, and abuse controls. The workspace-tier environment ingress default does not need an Enterprise upgrade before any public service can exist, and it must never be represented as an application authorization control.
- No service is attached to either environment. Consequently, no staging operator restriction, service ingress policy, application default-deny behavior, health/readiness endpoint, or public guest admission has been configured or tested.
