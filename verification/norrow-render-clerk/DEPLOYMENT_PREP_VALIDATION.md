...
Integration import note (2026-09-21): the donor `.dockerignore` omitted local
credential directories and `.p8` signing keys despite `COPY . .`. The local
Phase A/B integration branch adds exclusions for both and a governance
regression: expected-red 6 pass/1 fail before the exclusions, then 7/7 pass.
No Docker image was built, and the daemon socket remains inaccessible. This
static check is not proof of image contents, start, readiness or shutdown.
