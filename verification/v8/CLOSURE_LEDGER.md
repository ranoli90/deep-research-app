...
| CL-04 | P1 | persist-before-POST unit | closed mutating follow-up journal + callback failure matrix | Key computed at POST time; draft cleared after response | `runMutatingFollowUp` / assumptions / correction persist prepared→sent→accepted→adopted; 401/409/empty/superseded/double-tap unit | Device double-tap |
