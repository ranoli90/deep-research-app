...
Worktree: `/home/oranolio/Desktop/deep-norrow-render-clerk`; branch `codex/norrow-render-clerk-guest-auth`; base `bb80cfd2d2a3b14e224e516b165871fc60e58105`; tip `cdcaa534e2392b3ec07b39f2953d3d7926873793`. Keep the ordered lane history together: `8aa5388`, `9a1f2a6`, `734decd`, `bcb8b49`, `6b497cf`, `cdcaa53`. Do not merge it yourself; use the designated reviewer/integrator and retest the combined application.
...
Clerk is an unintegrated development/test instance (`ins_3JcB20N0jPnpXio97hZH6AhPMD6`): Native API enabled; Apple/Google only configuration-level; email disabled with no verification strategies; no redirects/origins. Email-code cannot be enabled through the documented BAPI and requires Clerk Dashboard/platform-management. Apple App Store Connect JWT was `401 NOT_AUTHORIZED`; Norrow Sign in with Apple Service ID/key, Google OAuth client/domain, provider callbacks, user/session/JWKS verification in the backend, webhooks, and deletion/revocation are all unverified or absent. Never add credentials or personal data to repo evidence.

Ownership fence: `apps/mobile/App.tsx` belongs to the `phase-a-state` lane. Do not touch it from this lane. The current mobile `guest-pending-action` helper is journal-only and must not be represented as a working sign-in experience. Before implementation, obtain explicit ownership handoff for API/contracts, persistence/migrations, backend auth adapter/HTTP boundaries, and any lockfile/device changes. The next integrator must implement the server-owned first-turn admission/sponsor budget, durable guest proof and conversation control, atomic claim into a member-owned continuation, local account mapping, webhook/deletion fanout, lifecycle/consent/HOLD fencing, and endpoint resolver matrix—not an account-field rewrite or client-side turn count.
