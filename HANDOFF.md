...
Next executable work: automatic receipt reconciliation and explicit confirmed/held cost reporting, native binary picker and remaining identity lifecycle. Then finish the general model gateway, criterion-linked completion, real correction reuse and held-out API/worker evaluation. Do not use the green fixture suite as evidence that these exist. Do not restore permissive publication behavior to preserve prose.
