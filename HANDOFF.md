...
Next executable work: structured generic model gateway and typed action execution results, including failed HTTP/unknown request outcomes and existing-attempt replay handling; current adapter can still return failed receipts without throwing. Keep live issuance off pending an enforceable route bound/current authorization. Native binary picker and remaining identity lifecycle also remain. Then finish the general model gateway, criterion-linked completion, real correction reuse and held-out API/worker evaluation. Do not use the green fixture suite as evidence that these exist. Do not restore permissive publication behavior to preserve prose.
