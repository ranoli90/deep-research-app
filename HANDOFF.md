...
Use only `/home/oranolio/Desktop/deep-v8-extraction-fixture-20260920` on local branch `codex/v8-extraction-fixture`, based on rejected candidate `19a0bd38`. Initial `cfe382b` fixed expanded query handling but was rejected for substring lookalikes. Bounded functional repair `5e76f51` requires normalized exact `offline editing` and `export` tokens, adds seven direct boundary controls, and hashes the executable helper in the artifact manifest. Production retrieval, coverage and publication code is unchanged.

From exact clean `5e76f51`, focused matched-pipeline **3/3**, full extraction **55/55**, and `pnpm verify` pass. Both artifacts embed the functional SHA, `clean_checkout`, and helper SHA-256 `576e06c8…`. A1 uses original/export-vendor/offline-review and B uses original/offline-vendor; both complete with two parser calls and both facts. Historical `cfe382b` artifacts are not final exact-SHA evidence. The reviewer’s unrelated W02 forced-GC timeout/focused **12/12** is separate and not a product or W08 failure. This is actual local extraction plus fabricated transport, not live semantic quality, strategy superiority, provider cost, hosted, native or release evidence. Do not merge or push. Exact prior red, final receipts, artifact hashes and rollback: `verification/v8/extraction-fixture-repair/RESULTS.json`.
...
The active semantic union is green: workspace `pnpm verify`, full core **392/392**, backend unit **247/247**, full mobile **504/504**, focused mobile **210/210**, workspace/mobile/backend types, boundaries, governance and both documentation validators pass. The former independent AG07 skip is now an active sibling-parser regression. The independent backend suite retains every assertion and uses a generic local snapshot-extra type so `readUrls` is checked without `any`, exclusion or suppression. Original blobs `909307e`, `d0e6287`, `1d4dea2` and `daaf359` remain recoverable from `4ba21a6`; final active tests supersede only stale harness/semantic/version expectations.
Worktree `/home/oranolio/Desktop/deep-v8-res02-20260920`, branch `codex/v8-res02-freshness`, base `3116f6e88074b51cd8f222e775ffcb2fdb8d4d9d`. Do not merge main/integration or push without owner review. Independent review rejected heads `3677bd83e028ac6f52d162cf707cd2094793c7c7`, `6eb29dd28fce399c78bfa6d54ff6a2e6b089dc9d`, and `7a4d70fb9df8d98157fe78c587a91ccb4d8545d9`; do not adopt those partial repairs alone. `requested-obligations.v2` binds optimistic coverage and the historical shortcut to persisted question-bound entity×fact obligations. Grammar-bound subject spans handle lowercase/case-mixed conjunction, comma, slash and multi-sentence forms; an unidentified plural subject fails closed. Assertion scope wins over incidental text; bare `expand` and inflected variants share one fact. The worker regression does not force an unresolved fixture.
Worktree `/home/oranolio/Desktop/deep-v8-res02-20260920`, branch `codex/v8-res02-freshness`, base `3116f6e88074b51cd8f222e775ffcb2fdb8d4d9d`. Do not merge main/integration or push without owner review. Independent review rejected heads `3677bd83e028ac6f52d162cf707cd2094793c7c7`, `6eb29dd28fce399c78bfa6d54ff6a2e6b089dc9d`, `7a4d70fb9df8d98157fe78c587a91ccb4d8545d9`, and `2cc678fa689ce1a1d4688d137a278f82399abf83`; do not adopt those partial repairs alone. `requested-obligations.v2` binds optimistic coverage and the historical shortcut to persisted question-bound entity×fact obligations. Grammar-bound subject spans handle lowercase/case-mixed conjunction, comma, slash and multi-sentence forms; an unidentified plural subject fails closed. Title-case fallback candidates that are only strict token subspans of a grammar-bound entity are suppressed, while explicitly requested `Alpha` and `Alpha Labs`, acronyms and single-word subjects remain distinct. Assertion scope wins over incidental text; bare `expand` and inflected variants share one fact. The worker regression does not force an unresolved fixture.

New policies are v4; v2/v3/v4 stored identities are immutable. v2 must be the default whole-run policy for the exact admitted original question and equal deterministic v2 recomputation; v3/v4 likewise must match deterministic recomputation under their stored version or resume fails closed. Natural undated workforce requests are current; fiscal-year ended/ending, fiscal/FY/quarter/end-of/fixed controls are historical unless explicit current/live wording wins. Full core **366/366**, focused **64/64**, case-mixed crossed/full-four production controls **2/2**, prior production worker siblings **8/8**, fresh-database v2/v3/v4 persistence **2/2**, workspace types, boundaries and governance pass. No paid/live/network route ran.

APPLE-01 remains external live readback evidence from the owner-authorized main-worktree continuation: App ID `app.deepresearch.mobile`, team `W6MVPS68ZU`, App Store Connect app `6814282574`, and version `1.0` in `PREPARE_FOR_SUBMISSION`. This integration did not use credentials or copy untracked portal screenshots. No signed build/TestFlight/release claim is made; `https://example.invalid` still blocks a functional production build.

Do not touch or clean main, `codex/phase-a-state`, integration visual-QA artifacts, source lanes, ports 8787/8788/8790, shared databases, or the MC-D01 hold. Nothing is pushed. Rollback reverts the final integration commit and imported local commits back to `4ba21a6`, while retaining historical parser/freshness readers, durable journals, Evidence Needs, consent/cancel/budget/ownership/deletion/publication fences, source evidence and unknown holds. Exact inventory and evidence: `verification/v8/recovery-rec01-rec02/RESULTS.json`.
Review local functional commits `9327b21fc3b7b602f34aa8607ada9b8b7d9add97`, `045076cc181336bb581f060de3b7ca6d14f4b7e9` and `1b7640ac1b90454fe625620d585afb8de5cc9703` from base `b818a1c07354b78d30cd8ed88bf1d37a36dfa70f` on `codex/v8-res04-sibling-journals`. Do not merge `main` or push from this lane.
Review local functional commits `9327b21fc3b7b602f34aa8607ada9b8b7d9add97`, `045076cc181336bb581f060de3b7ca6d14f4b7e9`, `1b7640ac1b90454fe625620d585afb8de5cc9703` and `62a374c9dadbe503610eaf974c4e0833e024de90` from base `b818a1c07354b78d30cd8ed88bf1d37a36dfa70f` on `codex/v8-res04-sibling-journals`. Do not merge `main` or push from this lane.

Review all three binders for exact digest/key recomputation, immutable request ID, known phases, required accepted run UUID, and forbidden accepted fields before acceptance. Confirmation additionally binds accepted identity to its parent run; a foreign UUID is held through parser, hydration, and mutation-runner paths with no POST/adoption. `bindPendingFollowUp` now parses every non-null record before unresolved/terminal branching, including `adopted`, `rejected` and `withdrawn`; damaged terminal-looking rows throw the device-cleanup hold before save/POST/adopt, while valid terminal rows permit a deliberate new request only after validation. Also inspect `canSubmit`, `startNewResearch`, routed follow-up and Android Back: every state gate parses all three non-null journal records before testing any phase, so a damaged sibling blocks admission/clearing and preserves its raw object and current run. No unverifiable legacy migration exists, and hydration does not delete the raw protected snapshot.

The first independent review rejected `9327b21` for foreign accepted confirmation identity. Integrated review rejected evidence head `2abcbd1` for phase-before-parse terminal replacement, then rejected `a7b6d52` because state admission/clearing gates remained phase-only. State repair `62a374c` reproduced **9/19** expected-red and now has state gates **21/21**, focused journals **151/151**, full mobile **468/468**, mobile typecheck and boundaries green. It awaits another independent rereview. No native claim and no DB/live/provider call. Rollback: revert `62a374c`, `1b7640a`, `045076c`, then `9327b21`; retain strict readers and held records because durable rows are not migrated or rewritten.
