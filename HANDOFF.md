...
W01–W09 is unfinished on branch `codex/v6-evidence-milestone`, based on `03fab6b9d6a04ce9fdaeb48636383757213f7242`. Safety/extraction checkpoint `0e9fc9b`, evidence commit `500ce1e`; report/support changes follow. Current `pnpm verify` passes and local integration is **139/139**. Read STATUS, `verification/v6/RESULTS.json` and `FINDING_RECHECKS.json`; historical fixture passes below are not general quality or release evidence. The legacy review-data validator remains failed on application command statuses.
...
Next executable work: complete legacy raw-file/derived-store deletion with durable cleanup and race protection; production identity/cross-object ownership; mobile secure token/request generations. Then finish real binary documents, strict typed actions/general model gateway, criterion-linked completion, real correction reuse and held-out API/worker evaluation. Do not use the green fixture suite as evidence that these exist. Do not restore permissive publication behavior to preserve prose.
