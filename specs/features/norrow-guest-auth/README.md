...
The following controls are mandatory additions to the named suites above. They are requirement/test linkage, not claimed execution evidence; their exact locks, tombstones, cleanup and fencing behavior is owned by ADR075.
...
| `AUTH-07` | Logout, same-account re-login, and account switch during auth/claim/adoption. Assert `principalEpoch` changes, stale callbacks fail, and the old action is preserved only as unsent draft. | `guest-pending-action`, physical native journey |
