...
The historical stop/read shortcut requires a proven atomic `requested-obligations.v1` shape derived from persisted criteria and linked questions: one question, entity and fact. Comma, slash, multi-sentence and deeper-subquestion compound forms remain non-atomic. A shared criterion cannot be satisfied by one relevant assertion unless supported assertions cover every requested entity and fact, even when model coverage is optimistic; its unresolved key remains connected to production Evidence Needs and limited-publication disclosure.
