...
New freshness writes use `criterion-freshness.v4`; persisted v2/v3/v4 identities remain immutable. v2 retains its admitted legacy meaning; v3/v4 restore only when their canonical row/JSON fields equal deterministic recomputation under the stored version. The authoritative criterion policy is persisted before retrieval for that key. v4 classifies each requested fact: explicit-now and undated present workforce facts remain current beside a historical founding fact, while fiscal/FY, quarter, end-of-period, fixed-date and `as of` facts stay historical.

The historical stop/read shortcut requires a proven atomic `requested-obligations.v2` shape derived from persisted criteria and linked questions: one question-bound entity×fact pair. Comma, slash, multi-sentence and deeper-subquestion compound forms remain non-atomic. A shared criterion cannot be satisfied unless supported assertions cover every requested pair, even when model coverage is optimistic. Assertion scope overrides incidental prose mentions; evidence for entity A/fact X and entity B/fact Y cannot cover the missing cross-pairs. The unresolved key remains connected to production Evidence Needs and limited-publication disclosure.
