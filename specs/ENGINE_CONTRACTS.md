...
Allowed actions: `clarify`, `search`, `fetch`, `extract_text`, `extract_table`, `inspect_visual`, `compare`, `calculate`, `verify`, `replan`, `synthesize`, `stop`. `extract_table` and `inspect_visual` are unavailable until their real adapters/tests exist; explicit capability flags reject use rather than fabricate results.
...
| POST /v1/attachments + finalize | Restricted upload, MIME/size validation, ownership, processing state and extraction limits. |
