...
- Initial repair `cfe382b` restored the expanded production-planner traces and passed W08/full extraction, but independent review rejected its substring classifier: `preoffline editing`, `offline editingly`, and `exportable` could be misclassified. Those dirty-tree artifacts remain historical and are not exact final evidence.
- Functional repair `5e76f51` tokenizes NFKC/lowercased Unicode words, requires adjacent exact `offline` + `editing`, and recognizes only exact token `export`. Seven direct unit controls cover negative lookalikes and positive case/whitespace/mixed forms. The extraction manifest hashes the executable `test/helpers/matched-model.ts`. Completed, two-parser, two-fact, support, coverage and publication assertions were not weakened; production code is unchanged.
- Exact clean `5e76f51`: `pnpm verify` passes core **392/392**, backend **254/254**, mobile **504/504**, governance **6/6**, types and boundaries. Focused W08 on fresh `deep_v8_fixture_review_focus_20260920_a92c1f06`: **3/3**, 118.20s, exit 0. Full extraction on fresh `deep_v8_fixture_review_full_20260920_d61e5b43`: **55/55** across 8 files, 567.26s, exit 0. Both artifacts embed `5e76f51`, `clean_checkout`, and helper SHA-256 `576e06c8…`. Actual offline parser, fabricated provider transport, no network or paid calls.
- Reviewer context: an unrelated W02 forced-GC full-suite timeout had a focused W02 result of **12/12**. It is not treated as a product or W08 failure. Evidence: `verification/v8/extraction-fixture-repair/RESULTS.json`. Rollback reverts `5e76f51` then `cfe382b`, leaving production consent/ownership/budget/deletion/publication/unknown-outcome behavior untouched.
...
- Recovery head `4ba21a6` preserves the independent union and original blobs. Imported RES-01 functional `a4f66d2`/`0b87ae6`/`e095d72`, RES-04 functional `9327b21`/`045076c`, and approved RES-02/03/05 `3677bd8`/`6eb29dd`/`7a4d70f`. Patch-equivalent b818/55ea/3116 bases were not replayed. Global status/handoff/ledger conflicts were deliberately deferred and synthesized here once; functional/spec/test files merged without production conflicts.
- The BB-01 add/add suite was resolved assertion-by-assertion. Its original `909307e` blob remains in `4ba21a6`; active tests include all original cases plus cancellation, consent-only and account-race controls. Integration corrected the assumption-confirm harness to return the parent run ID actually returned by the production API. RES-04 strict parsing then accepted legitimate same-parent confirmation and rejected a foreign accepted identity without POST or clear.
- Independent BB-02 mobile/core/backend blobs `d0e6287`, `1d4dea2` and `daaf359` remain in ancestry. Active core assertions now apply the approved present-workforce/current and explicit-as-of historical semantics, admit v4 while retaining v2/v3 readers, and pass the explicit atomic-override input. The active backend suite retains all assertions and fixes only generic snapshot-extra typing; no `any`, skip, exclusion or `ts-nocheck` was added. The deferred AG07 skip was replaced with active malformed assumption/correction parser checks.
- RES-02 review rejected `3677bd8` for forced-fixture/example-shaped coverage, then rejected `6eb29dd` for wrong-scope/crossed-pair coverage. Final `7a4d70f` uses question-bound entity×fact pairs and production unresolved Evidence Needs/limited disclosure. Independent lane evidence on fresh isolated databases passed worker siblings **8/8** and policy replay/tamper **2/2**. Integration did not reuse or create a database.
- Integrated checks: focused mobile **210/210**; full mobile **504/504**; full core **392/392**; backend unit **247/247**; governance **6/6**; workspace/mobile/backend types and boundaries pass; both documentation validators pass. `pnpm verify` is green. No paid/live/provider/native/EAS/GHA command ran.
- APPLE-01 is a prior owner-authorized external readback, not rerun here: team `W6MVPS68ZU`, App ID `app.deepresearch.mobile` with Sign in with Apple, App Store Connect app `6814282574`, version `1.0`, state `PREPARE_FOR_SUBMISSION`, and HTTP-200 listing readback. Main-worktree screenshots remain external/untracked and were not copied. Zero builds and the `https://example.invalid` production endpoint mean no signed-build, TestFlight, submission or release claim.
- Artifact: `verification/v8/recovery-rec01-rec02/RESULTS.json`. Rollback returns this local branch to `4ba21a6` while preserving all historical readers, durable accepted identities, unresolved Evidence Needs, source evidence, consent/cancel/budget/ownership/deletion/publication fences and financial/unknown holds.
- Base `3116f6e88074b51cd8f222e775ffcb2fdb8d4d9d`, branch `codex/v8-res02-freshness`, isolated worktree only; no merge/push/live provider route.
- Independent review rejected `3677bd83e028ac6f52d162cf707cd2094793c7c7`: its worker fixture forced the expected unresolved key while the production coverage resolver could accept one shared-key assertion as complete; punctuation heuristics missed comma lists; workforce fixed periods and v3/v4 semantic replay were incomplete.
- Independent rereview also rejected `6eb29dd28fce399c78bfa6d54ff6a2e6b089dc9d`: incidental mention could cover an entity contrary to assertion scope, entity and fact sets were not paired, and bare `expand` was unrecognized.
- Integrated review rejected `7a4d70fb9df8d98157fe78c587a91ccb4d8545d9`: lowercase compound subjects were not extracted, fixed fiscal-period variants were incomplete, and self-consistent wrong v2 semantics could restore.
- Integrated rereview rejected `2cc678fa689ce1a1d4688d137a278f82399abf83`: case-mixed grammar correctly found `ARDENT labs`, but title-case fallback also emitted strict-prefix alias `ARDENT`, creating an impossible fifth obligation and a false incomplete result.
- `requested-obligations.v2` derives question-bound entity×fact obligations from persisted linked questions, criterion scope/provenance and the immutable original question. Grammar-bound subject spans work independently of capitalization; ambiguous plural subjects fail closed. Assertion scope is authoritative. Optimistic review cannot clear a compound key until relevant supported assertions cover every requested pair. The same representation gates the bounded historical shortcut; atomic controls remain eligible.
- `criterion-freshness.v4` is the new-write identity. Stored rows are insert-once and owner scoped. A legacy v2 row must be the default policy for the exact admitted original question and equal deterministic v2 recomputation. v3/v4 require exact canonical keys, matching columns and equality to deterministic recomputation under their stored version. Every mismatch fails closed without overwrite.
- Final-repair full research-core 365/365; focused coverage/held-out/freshness 63/63; workspace typechecks, boundaries and governance EXIT 0.
- Isolated PostgreSQL/fabricated transport: all historical worker siblings 8/8 on `deep_res020305_final_lower_v2_20260920_1955`; lowercase compound, wrong-scope and crossed-pair coverage remain missing Evidence Needs with limited publication, while fiscal/current and atomic controls retain their semantics. Authoritative first-write and valid/tampered v2/v3/v4 replay passed 2/2 on `deep_res020305_persistence_v2_20260920_1950`.
- Alias repair: grammar-bound entities suppress strict token-subspan fallback aliases deterministically, without coalescing explicit overlapping names, acronyms or single-word subjects. Focused core 64/64 and full core 366/366 pass. Fresh PostgreSQL `deep_res02_alias_final_20260920_2016` passes crossed-pair limited and case-mixed full-four-pair complete controls 2/2.
- Rollback stops new v4 assignment but retains all historical readers/rows, Evidence Needs and consent/cancel/budget/deletion/publication fences.
- Scope: local isolated branch `codex/v8-res04-sibling-journals`, base `b818a1c07354b78d30cd8ed88bf1d37a36dfa70f`, implementation `9327b21fc3b7b602f34aa8607ada9b8b7d9add97` (tree `c6d0c2d94a3d80bbede78ae6255b3cc631e1204f`). Not pushed or merged.
- Initial registered mobile test on test-only changes: EXIT 1, 34 expected RES-04 failures and 403 passes. Failures showed digest/request/phase coercion plus permissive restart hydration for both sibling journals.
- `readPendingAssumptions` / `readPendingCorrection`: lowercase SHA-256 format, recomputed digest and idempotency key, UUID request identity, exact phase set, accepted-field rules, and accepted run UUID. No parser mints identity. Unverifiable legacy rows have no supported migration and remain held for cleanup.
- `bindPendingAssumptions` / `bindPendingCorrection` parse every extant record before phase branching. Corrupt or ambiguous terminal-looking state cannot be replaced by a fresh postable identity. Accepted protected-cache rows hydrate and adopt without POST; damaged rows fail hydration without altering the raw snapshot.
- Behavioral regressions replace the BB-03 source-presence anti-fix and cover assumption/correction malformed matrices, same-run confirmation, accepted child recovery, identity stability, no second POST, and restart damage. The independent AG07 skipped file is not in this branch; integration must replace that marker with this positive suite.
- Registered verification on exact implementation commit: `pnpm --filter @deep/mobile test` EXIT 0 (**52 files / 437 tests**); `pnpm --filter @deep/mobile typecheck` EXIT 0; `node scripts/check-boundaries.mjs` EXIT 0 (`boundaries=ok`). The typecheck initially exposed the base test's `outcome` excess-property error; the helper input type now admits that existing run field without runtime change.
- Independent review rejected the initial head because a same-run confirmation could hydrate/adopt a foreign well-formed accepted UUID. Added parser/hydration/production-runner coverage reproduced **1 failure / 437 passes** before the repair. Commit `045076cc181336bb581f060de3b7ca6d14f4b7e9` binds accepted/adopted confirmation identity to the parent (case-insensitive UUID spelling), holds damage, and performs no POST/adoption. Exact repaired commit: mobile **52 files / 438 tests**, typecheck EXIT 0, boundaries `ok`.
- Integrated review rejected evidence head `2abcbd1107f471403391ed647f33a6c3675e7f5e`: `bindPendingFollowUp` called phase-only `unresolvedFollowUp` before parsing, so malformed `adopted`, `rejected` and `withdrawn` rows minted and posted a fresh identity. The exact test-only reproduction failed **6/23** and showed a new UUID plus POST/adoption for every terminal phase.
- Commit `1b7640ac1b90454fe625620d585afb8de5cc9703` parses every extant follow-up record before phase branching. Damaged terminal rows now throw before any save/POST/adopt; valid terminal rows permit a new request only after strict validation. Focused journal suites **130/130**, full mobile **447/447**, mobile typecheck and boundaries pass. Independent rereview remains required; this lane does not self-approve.
- The next review rejected evidence head `a7b6d52cdf15400abe68dc6b9e240b0ae0373e99`: `canSubmit` and `startNewResearch` still inspected journal phases directly. Damaged terminal follow-up, assumption and correction rows therefore authorized App admission or were cleared by New Research, routed follow-up and Android Back. Exact test-only reproduction failed **9/19** across all three journal types and terminal phases.
- Commit `62a374c9dadbe503610eaf974c4e0833e024de90` strict-parses all three non-null journals before any state phase check. Damage returns a cleanup block while retaining the raw in-memory record and current run; an earlier unresolved journal cannot conceal a damaged later sibling. Valid terminal rows remain replaceable after validation. State-gate regressions **21/21**, focused journal suites **151/151**, full mobile **468/468**, mobile typecheck and boundaries pass. Another independent rereview remains required; this lane does not self-approve.
- Evidence-document checks after recording the result: `python3 scripts/validate_review.py` EXIT 0 (`ok: true`); review-validator unittest discovery EXIT 0 (**16/16**).
- Environment/artifact: local Node/pnpm, offline frozen dependency materialization with zero downloads; no database, network, native runtime, provider, or paid call. Evidence: `verification/v8/res04-sibling-journals/RESULTS.json`.
- Rollback: revert `62a374c`, `1b7640a`, `045076c`, then `9327b21`, while retaining strict readers and held records. No schema, dependency, stored-row rewrite, financial state, or remote state changed.
