...
`evaluateDiscoveryContinuation` must receive adopted sources, failed-query counts, novelty vs prior source count, evaluated freshness, and attempted source classes. Passing `sources:[]`, `novelty:1`, `freshnessUnmet:false`, `priorFailedQueries:0` makes independence and class-change dead. When evidence is weak, duplicative, or stale, call `nextSourceClass` and pass that class into search expansion. Do not map `freshnessUnmet` onto every criterion key for historical facts; `discoveryContinuationGaps` must keep `unresolvedConsequential` false after supported past-tense public facts so coverage-incomplete does not burn `DEEP_DISCOVERY_CEILING`.
...
Account and source deletion must scrub new retrieval columns (`origin_relation`, `publication_date`) as well as the new tables. Canonical integration migrations: `042_model_portfolio.sql` then `043_retrieval_intelligence.sql` then `044_query_authorization_proof.sql` then `045_model_operation_attempts.sql` then `046_research_controller_state.sql` then `050_phase_a_invariants.sql` (047–049 belong to other lanes). Isolated worker DBs may still record historical `042_retrieval_intelligence` / `043_model_portfolio` / `044_retrieval_intelligence` ids. `CREATE TABLE IF NOT EXISTS` does not add uniqueness later; add `CREATE UNIQUE INDEX IF NOT EXISTS` for `ON CONFLICT`. Additive 050 owner/spend CHECKs/FKs are `NOT VALID` and must not rewrite historical rows.
