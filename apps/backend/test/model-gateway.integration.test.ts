...
async function runCase(test: (x: { runId: string; accountId: string; fence: number; session: ReturnType<typeof fencedSession>; config: ReturnType<typeof loadConfig> }) => Promise<void>,initialQuestion=question,admit?:{modelPolicyId?:ModelPolicyId}) {
  ...
  try { await test({ runId, accountId, fence, session, config }); }
    ...
    await withTx(pool, async (db) => {
      ...
      for (const table of ["query_authorizations","source_origin_links","criterion_freshness_policies","document_web_reconciliations","search_coverage","selection_inventory_checks","evidence_selections", "extraction_receipts", "evidence_artifacts", "model_operation_attempts", "provider_intents", "claim_revisions", "claims", "run_actions", "run_leases", "run_dispatch_outbox", "run_events", "reservations", "passages", "sources"]) {
...
describe("W05 evidence-bound arbitrary assertion extraction", () => {
  ...
  it("rejects an invented evidence binding instead of creating assertions", async () => runCase(async (x) => {
    const prepared = await extractionCase(x);
    const fake = { passageId: prepared.p.passageId, start: 0, end: 41, quote: "invented sentence that is not on the page" };
    prepared.output.candidates[0]!.evidence = [fake];
    prepared.output.assertions[0]!.evidence = [fake];
    globalThis.fetch = vi.fn(async () => response(prepared.output)) as typeof fetch;
  ...
  it("retargets an extraction quote whose passage id is wrong but whose text is uniquely owned", async () => runCase(async (x) => {
    const prepared = await extractionCase(x);
    const invented = crypto.randomUUID();
    prepared.output.candidates[0]!.evidence[0]!.passageId = invented;
    prepared.output.assertions[0]!.evidence[0]!.passageId = invented;
    globalThis.fetch = vi.fn(async () => response(prepared.output)) as typeof fetch;
    const result = await extractEvidenceAssertions(pool,x.config,x.session,prepared.args);
    expect(result.kind).toBe("extraction");
    if (result.kind !== "extraction") return;
    expect(result.output.assertions[0]!.evidence[0]!.passageId).toBe(prepared.p.passageId);
    expect(result.output.assertions[0]!.evidence[0]!.quote.length).toBeGreaterThan(0);
  }));
...
describe("W05 substantive support execution and persisted revisions",()=>{
  it("performs and stores scoped checks with stable claim revisions on replay",async()=>runCase(async(x)=>{
    ...
    expect(rows.rows[0]).toMatchObject({evidence_digest:expect.stringMatching(/^[a-f0-9]{64}$/),scope_digest:expect.stringMatching(/^[a-f0-9]{64}$/),checker_version:"scoped-support.v8"});
...
describe("W05 generic writer, exact final wording and canonical publication",()=>{
  ...
  it("adds a new checker result without overwriting an older version or paying again",async()=>runCase(async(x)=>{
    ...
    expect(second).toEqual({...first,reused:true});
    expect((await pool.query("SELECT checker_version FROM scoped_support_results WHERE run_id=$1 ORDER BY checker_version",[x.runId])).rows).toEqual([{checker_version:"scoped-support.v1"},{checker_version:"scoped-support.v8"}]);
    expect(globalThis.fetch).toHaveBeenCalledTimes(1);
  }));
});
