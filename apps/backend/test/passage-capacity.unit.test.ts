...
import { estimateTokens, operationBudget } from "../src/adapters/model/token-budget.js";
...
it("W02 bounded capacity keeps each entire passage, pricing and serialized request limits",()=>{
 ...
 expect(JSON.parse(body.messages[1].content).passages).toEqual(input.passages);
 expect(estimateTokens(request.body)).toBeLessThanOrEqual(STRUCTURED_MODEL_POLICY.contextTokens);
 expect(body.max_tokens).toBe(operationBudget("extract_assertions", STRUCTURED_MODEL_POLICY.id).maxOutputTokens);
 expect(body.provider.max_price).toEqual({prompt:0.15,completion:0.6,request:0});expect(STRUCTURED_CALL_RESERVE_MICRO).toBe(21658);
 ...
 // Token-aware admission: 25×7000 chars is under the extract input budget; 25×11000 is not.
 expect(()=>prepareModelRequest("extract_assertions",context(25,7000))).not.toThrow();
