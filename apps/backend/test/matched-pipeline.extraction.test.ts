...
afterAll(async()=>{
 ...
 const versions=await Promise.all(["apps/backend/src/adapters/model/prompts.ts","apps/backend/src/ports/model-policy.ts","packages/contracts/src/research-model.ts","evals/matched-pipeline/protocol.json","apps/backend/src/worker/structured-research.ts","apps/backend/src/worker/attachment-ingestion.ts","apps/backend/test/helpers/matched-model.ts","apps/backend/test/matched-pipeline.extraction.test.ts"].map(async path=>({path,sha256:digest(await readFile(new URL(`../../../${path}`,import.meta.url)))})));
