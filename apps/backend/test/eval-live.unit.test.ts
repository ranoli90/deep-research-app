...
it("unsupported frozen mode records every unrun step before any driver activity",async()=>{const d=driver(),events:unknown[]=[];const r=await runMatched(plan({...grant(),sourceMode:"frozen_supplied_document"}),d,async v=>{events.push(v)});expect(r.halted).toBe("frozen_document_mode_not_implemented");expect(events.filter((v:any)=>v.event==="unrun")).toHaveLength(4);expect(d.exposure).not.toHaveBeenCalled();expect(d.admit).not.toHaveBeenCalled();});
