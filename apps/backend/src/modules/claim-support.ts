...
import { CALCULATION_REPORT_VERSION, SCOPED_SUPPORT_VERSION, passageSupportsClaim, type StoredClaim, type StoredPassage, type ReportDerivationContext } from "@deep/research-core";
...
export async function persistCheckedClaims(db: Queryable, args: {
  ...
  for (const claim of args.claims.filter((c) => referenced.has(c.id))) {
    ...
    const calculation=args.calculationApprovals?.get(claim.id);
    ...
    if(scoped) {
      ...
      const updated=await db.query("UPDATE claims SET support_status=CASE WHEN type IN ('inference','calculation') THEN 'inference' ELSE 'direct' END WHERE id=$1 AND account_id=$2 AND run_id=$3 AND text=$4",[claim.id,args.accountId,args.report.runId,claim.text]);
      ...
      for(const passageId of scoped.passageIds) await db.query(`INSERT INTO claim_evidence(claim_id,passage_id,relation,checker_version,decision,explanation)
        VALUES($1,$2,'supports',$3,'supports',$4) ON CONFLICT DO NOTHING`,[claim.id,passageId,calculation?CALCULATION_REPORT_VERSION:SCOPED_SUPPORT_VERSION,calculation?'Input evidence supports an exact arithmetic derivation; not direct source wording.':'Current scoped support result revalidated at publication']);
      continue;
    }
