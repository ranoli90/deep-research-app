...
export async function deleteSourceForAccount(pool:pg.Pool|pg.PoolClient,accountId:string,sourceId:string,
 ...
 if(authority){
  ...
  await db.query(`UPDATE source_versions SET final_locator='[deleted]',content_hash=NULL,mime=NULL,access_level='blocked',
   quality_warnings='[]',text_coverage=NULL,artifact_ptr=NULL,effective_date=NULL,applicable_version=NULL WHERE account_id=$1 AND id=ANY($2::uuid[])`,[accountId,versions]);
