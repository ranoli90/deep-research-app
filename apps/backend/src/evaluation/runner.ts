...
import type { RegisteredPlan } from "./authorization.js";
export type Step={id:string;taskId:string;repeat:number;arm:"A1"|"B";kind:"original"|"correction"|"full_rerun";question:string;parentStepId?:string;idempotencyKey:string};
...
function stepKey(approvalId:string,stepId:string){const h=createHash("sha256").update(`matched-evaluation:${approvalId}:${stepId}`).digest("hex");return `${h.slice(0,8)}-${h.slice(8,12)}-8${h.slice(13,16)}-a${h.slice(17,20)}-${h.slice(20,32)}`;}
export function stepsFor(plan:RegisteredPlan):Step[]{
 ...
 }return steps.map(step=>({...step,idempotencyKey:stepKey(plan.authorization.approvalId,step.id)}));
}
...
export async function runMatched(plan:RegisteredPlan,driver:Driver,journal:Journal){
 ...
 if(plan.authorization.sourceMode!=="live_discovery")halted="frozen_document_mode_not_implemented";
