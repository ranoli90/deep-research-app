...
export function registeredPlan(grant:Authorization,files:{protocol:string;freeze:string;tasks:string;sources:string}):RegisteredPlan {
 ...
 const tasks:TaskProjection[]=catalog.tasks.map((t:unknown,index:number)=>{const v=z.object({id:z.string(),question:z.string().min(1).max(20000),correctedQuestion:z.string().min(1).max(20000)}).parse(t);return {...v,index,repetitions:protocol.repetitions.selectedThreeRepeats.includes(v.id)?3:1};});
