...
export const ExtractedDocument = z.object({
  version: z.literal("trafilatura-2.2.0/structure-v1"), digest: z.string().regex(/^[a-f0-9]{64}$/),
  ...
  blocks: z.array(z.object({ kind: z.enum(["text", "heading", "code", "table"]),
    locator: z.string().max(200), text: z.string().max(1_000_000), rows: z.array(z.array(Cell).max(1000)).max(10000) }).strict()).max(10000),
}).strict();
...
export function extractOffline(bytes: Buffer, mime: string, options: { signal?: AbortSignal; runtime?: string } = {}): Promise<ExtractedDocument> {
  if (bytes.length > MAX_FETCH_BYTES) return Promise.reject(new Error("extraction_too_large"));
