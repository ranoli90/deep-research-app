...
export async function performModelOperation<K extends ResearchModelOperation>(pool: pg.Pool, config: AppConfig, session: FencedSession, args: {
  ...
  if (!attempt.issue) {
    const cached = await session.write((db) => loadModelOperation(db, attempt.intentId, args.runId, args.accountId, request.digest));
  ...
  await session.write(async (db) => {
    ...
    await saveModelOperation(db, { ...args, intentId: attempt.intentId, request, result });
  });
