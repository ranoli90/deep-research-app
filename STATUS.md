...
Isolated branch `codex/v8-extraction-fixture` from rejected recovery candidate `19a0bd38`. The prior exact-SHA extraction gate was **54/55**: source-class expansion produced `export vendor documentation` and `offline editing independent review`, but the fabricated transport recognized only exact `offline editing`, returned `export.html` for every search, and correctly published A1 as `completed_with_limitations`. Production coverage/publication behavior was not changed.

The fabricated matched transport now selects `offline.html` for an offline-only expanded query while the mixed opening query remains `export.html`. Exact trace assertions pin A1 to original/export-vendor/offline-review and B to original/offline-vendor. Existing completed, two-parser, two-fact, support and publication assertions remain unchanged. Focused W08 is **3/3** on fresh `deep_v8_fixture_20260920_b6d49c1a`; full extraction is **55/55** on fresh `deep_v8_fixture_full_20260920_4d37a8c2`; `pnpm verify` passes core **392/392**, backend **247/247**, mobile **504/504**, governance **6/6**, types and boundaries. Fabricated transports, paid zero, no network, no semantic-quality or arm-superiority claim. Evidence: `verification/v8/extraction-fixture-repair/RESULTS.json`. Not merged or pushed. Rollback reverts this fixture-only commit and intentionally restores the known control failure; production gates are unaffected.
