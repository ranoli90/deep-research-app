...
Isolated worktree `/home/oranolio/Desktop/deep-v8-res04-20260920`, branch `codex/v8-res04-sibling-journals`, base `b818a1c07354b78d30cd8ed88bf1d37a36dfa70f`, initial implementation `9327b21fc3b7b602f34aa8607ada9b8b7d9add97`, foreign-confirmation repair `045076cc181336bb581f060de3b7ca6d14f4b7e9`, terminal-binder repair `1b7640ac1b90454fe625620d585afb8de5cc9703`. **Local only; not merged to `main` and not pushed.**

Follow-up, assumption and correction durable records now match the BB-03 fail-closed parser boundary: lowercase SHA-256, recomputed payload/key binding, immutable UUID request identity, exact known phase, phase-specific accepted fields, and valid accepted run UUID. Confirmation is same-run: an accepted/adopted confirmation naming any other well-formed UUID is damaged and held rather than adopted or cleared. Every extant record is parsed before terminal/unresolved branching. In particular, a damaged follow-up labeled `adopted`, `rejected` or `withdrawn` cannot be replaced with a new request identity or authorize save, POST, adoption or silent clear. Valid terminal rows may allow a deliberate new request only after strict validation. Current unversioned, fully bound records remain readable; legacy records missing identity are deliberately unsupported and held for device cleanup rather than implicitly migrated. Protected-cache damage remains intact while hydration fails.

The temporary BB-03 source-presence assertion is behavioral. RES-04 covers corruption matrices, accepted restart/no-second-POST recovery, same-run confirmation, foreign-confirm hold, storage-damage restart and all three follow-up terminal phases. The first independent review rejected foreign confirmation at `9327b21`; the integrated review then rejected evidence head `2abcbd1` because `bindPendingFollowUp` inspected terminal phase before parsing. The exact second reproduction failed **6/23** before `1b7640a` and passes afterward. Current verification: focused durable journals **130/130**, full mobile **447/447**, mobile typecheck EXIT 0, boundaries `ok`. No DB, network, provider, native, or paid call. This repair awaits independent rereview; it is not self-approved. Rollback: revert `1b7640a`, then `045076c`, then `9327b21`, while retaining strict readers and held records; no schema or dependency rollback is needed.
