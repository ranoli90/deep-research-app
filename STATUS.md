## Norrow Render + Clerk guest-first checkpoint — 2026-09-20

At checkpoint inspection this isolated lane is `codex/norrow-render-clerk-guest-auth`, based on integration `bb80cfd2d2a3b14e224e516b165871fc60e58105`, with application-source HEAD `724637a10f126b1d63191c7d05a06587e2862be4`. Since stale source checkpoint `3bc6467`, `3c13d1d`, `2573daa`, and `724637a` added reconciliation correlation and claim-scope fences (with documentation-only `431fbd6` also in the local sequence). A fresh remote readback found remote Norrow at `cebbfd502af49cb8929ecba0eb7fa8ec76a6bbb8`, integration at `bb80cfd2d2a3b14e224e516b165871fc60e58105`, and main at `d89afe3d8ae4c57682cb603d79c2c23605c4e9e0`; local source is eight commits ahead of the remote lane and fifteen ahead of integration. It is **not merged** to integration or `main`, and there is no deployed app build. The protocol received independent approval only as additive implementation authority; it is not evidence that the backend, database, Clerk, or native product path exists.
...
The branch now contains a durable journal expiry/terminal-state and claim-scope repair (`45e9e2f`, `3bc6467`, `3c13d1d`, `2573daa`, `724637a`). Independent focused review accepted its control at **17/17** (`pnpm --filter @deep/mobile test -- guest-pending-action`); that is not proof of external exactly-once execution. The `92866cf` sign-in sheet remains a presentational seam and is rejected for application integration until the Phase A state owner supplies an adapter that derives visibility from a decodable `AUTH_REQUIRED_NEXT_TURN` journal, invokes only actually-configured Clerk providers, persist-before-unsafe-work journal transitions, verified scoped claim under real principal/view/control epochs, and non-destructive dismissal/explicit-resume behavior. `apps/mobile/App.tsx` remains owned by the `phase-a-state` lane. The proposed packet is the nontracked local [coordination record](/home/oranolio/Desktop/Deep/.git/norrow-render-clerk-coordination.md), at base `bb80cfd2d2a3b14e224e516b165871fc60e58105`/then-tip `cebbfd502af49cb8929ecba0eb7fa8ec76a6bbb8`; it has no live owner contact or recorded acceptance. It assigns the mobile state seam to its existing owner and requires a backend owner to accept `apps/backend/src/api/app.ts`, verifier/authority/admission/claim/lifecycle, access/runs/worker fences, strict contracts, migration after `051_closure_identities.sql`, and endpoint/race/webhook tests. Until that transfer is recorded, API/contracts, persistence/migrations, Clerk verification/webhooks, lockfile, and devices remain untouched by this lane. Do not enable guest issuance or substitute a frontend counter: the approved protocol requires server-owned admission, an atomic sponsor ledger, immutable execution ownership, scoped claim, consent/deletion/cancellation/HOLD fences, and exactly-once continuation.

Evidence executed in this lane: authenticated Render `GET /v1/projects/{id}` and `GET /v1/postgres/{id}` readbacks (exit 0); authenticated Clerk `GET /v1/instance` and public JWKS reachability (exit 0); Git inventory (exit 0); and the focused journal suite **17/17** at `724637a`, all on 2026-09-20 from the local operator environment. Both Render recovery endpoints now read `AVAILABLE`, which is provider recovery availability only; no backup restore, failover, app recovery, or service has run. The typecheck failure at `apps/mobile/test/query-authorization.test.ts:67` is a historical run at stale source `3bc6467`, not a test result at this source head; current combined mobile verification remains blocked until re-run/owner repair. No migration, native flow, hosted app journey, provider login, load, restore, or 132-scenario acceptance test was run by this lane. Database list-price estimate is USD 106.50/month before tax/workspace/services/model vendors; it is an estimate, not a spend cap or invoice. Rollback is a separately authorized Render-only operation: first prove no data/services/retention obligations, then explicitly remove the two databases before the project and read back absence; never erase claim/tombstone/hold data to roll back an eventual application implementation.
