...
The authorized V6 W01–W09 internal milestone remains **incomplete**. Branch `codex/v6-evidence-milestone`; latest reviewed implementation `0602f96efff0eaa576e7654f08b226ef20108de5` (authenticated route preflight), following `b15c3d85a9fe9911db2323dba5de3a369359591d` (whole-passage capacity128 and HTML structure-v3). Preserve the application and both untracked user ZIPs. No push/deployment/paid calls are authorized. Current paid allowance is unknown.
...
| W04 | Actual isolated digital PDF/HTML extraction, binary upload, page/geometry/source metadata and explicit partial coverage; synthetic Android picker/upload/consent/deletion proof | Broader difficult documents/tables/columns beyond current33 actual controls, OCR/layout, standalone resolved native graph and broader source-reader acceptance |
...
| W07 | Research/Library/profile, document picker, source sheet, bibliography, cancellation/reopen, protected drafts/cache, admission/verification/deletion recovery, reading anchors, profile extraction, fresh configured-route preflight | Complete current authenticated native research→source→correction→share journey, native source deletion, account switches/reconnect/accessibility and remaining responsibility-based refactoring |
...
Exact-current clean-checkout proof passed. Scope audit identified executable evaluation-runner and new-document correction gaps; bounded implementation is in progress. After unlock, clean the bound synthetic device session and execute current native acceptance. Real paid model evaluation requires a current explicit allowance; hosted/release work requires separate authority.
