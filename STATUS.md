...
Repair evidence: full research-core **365/365**, focused semantic coverage/freshness **63/63**, research-core/backend/workspace typechecks, boundaries and governance pass. A fresh isolated PostgreSQL run passes all historical siblings **8/8**, including the lowercase compound, wrong-scope mention, crossed entity×fact pairs, fiscal/current controls, atomic controls and valid v2 resume (`deep_res020305_final_lower_v2_20260920_1955`). The incomplete compound paths keep the Evidence Need missing and publish only `completed_with_limitations` with exact unresolved question/criterion disclosures. A separate fresh database passes authoritative first-write plus valid/tampered v2/v3/v4 replay **2/2** (`deep_res020305_persistence_v2_20260920_1950`). Fabricated nonbillable transport only; no live/network provider call. Full `pnpm verify` and the full PostgreSQL suite are not claimed.
