...
This isolated lane is `codex/norrow-render-clerk-guest-auth`, based on current integration base `bb80cfd2d2a3b14e224e516b165871fc60e58105`; it is six commits ahead, in this exact order: `8aa5388`, `9a1f2a6`, `734decd`, `bcb8b49`, `6b497cf`, `cdcaa53`. It is **not merged** to integration or `main`. The protocol received independent approval only as an additive implementation authority; it is not evidence that the backend, database, Clerk, or native product path exists.
...
Clerk instance `ins_3JcB20N0jPnpXio97hZH6AhPMD6` is a development/test instance. Native API is enabled; Apple and Google factors are configuration-level only. Email address is disabled and email verification strategies are empty. The documented BAPI cannot enable email-code, so a Clerk Dashboard/platform-management action is required before email authentication can be configured. No redirect URLs/origins are configured; no provider flow, login, webhook, user, claim, or native journey was exercised. Apple App Store Connect JWT probe returned `401 NOT_AUTHORIZED`; no Norrow Sign in with Apple Service ID/key, Google OAuth client/domain, or hosted provider callback is verified. These are **BLOCKED/CONFIGURED_NOT_EXERCISED**, not an authentication integration.

The only code on this branch is the protected mobile pending-action journal and its auth-epoch fix; it has not been connected to the application flow. `apps/mobile/App.tsx` is actively owned by the `phase-a-state` lane. Backend API/contracts, persistence/migrations, and the Clerk adapter/verification/webhook paths have no handoff in this lane and must remain exclusively owned before edits. Do not enable guest issuance or substitute a frontend counter: the approved protocol requires server-owned admission, an atomic sponsor ledger, immutable execution ownership, scoped claim, consent/deletion/cancellation/HOLD fences, and exactly-once continuation.

Evidence executed in this lane: authenticated Render `GET /v1/projects/{id}` and `GET /v1/postgres/{id}` readbacks (exit 0); authenticated Clerk `GET /v1/instance` and public JWKS reachability (exit 0); and `git log`/`git show` commit inventory (exit 0), all on 2026-09-20 from the local operator environment. No source test, migration, native flow, hosted app journey, provider login, or load/recovery test was run by this lane. Database list-price estimate is USD 106.50/month before tax/workspace/services/model vendors; it is an estimate, not a spend cap or invoice. Rollback is a separately authorized Render-only operation: first prove no data/services/retention obligations, then explicitly remove the two databases before the project and read back absence; never erase claim/tombstone/hold data to roll back an eventual application implementation.
