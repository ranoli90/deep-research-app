## W08 matched-pipeline expanded-query fixture repair — 2026-09-20

Isolated branch `codex/v8-extraction-fixture` from rejected recovery candidate `19a0bd38`. The prior exact-SHA extraction gate was **54/55**: source-class expansion produced `export vendor documentation` and `offline editing independent review`, but the fabricated transport recognized only exact `offline editing`, returned `export.html` for every search, and correctly published A1 as `completed_with_limitations`. Initial repair `cfe382b` was independently rejected because substring matching also accepted `preoffline editing`, `offline editingly`, and `exportable` lookalikes.

Functional repair `5e76f51` uses normalized exact tokens: only adjacent `offline` + `editing` form the offline phrase, and only token `export` makes it mixed. Seven direct unit controls cover lookalikes, case and whitespace. Exact trace assertions still pin A1 to original/export-vendor/offline-review and B to original/offline-vendor; completed, two-parser, two-fact, support, coverage and publication assertions are unchanged. The artifact version manifest now hashes the executable matched-model helper. From exact clean commit `5e76f51`, focused W08 is **3/3** on fresh `deep_v8_fixture_review_focus_20260920_a92c1f06`; full extraction is **55/55** on fresh `deep_v8_fixture_review_full_20260920_d61e5b43`; `pnpm verify` passes core **392/392**, backend **254/254**, mobile **504/504**, governance **6/6**, types and boundaries. Fabricated transports, paid zero, no network, no semantic-quality or arm-superiority claim. Historical dirty `cfe382b` artifacts are retained but not cited as exact final evidence. Evidence: `verification/v8/extraction-fixture-repair/RESULTS.json`. Not merged or pushed. Rollback reverts `5e76f51` then `cfe382b`; production gates are unaffected.

The independent reviewer separately observed a W02 forced-GC full-suite timeout with its focused W02 control **12/12**. That is unrelated to this W08 fixture repair and is not classified here as a product or W08 failure.

## V8 recovery integration — 2026-09-20

The isolated local branch `codex/v8-recovery-20260920` remains based on refreshed integration `bb80cfd2d2a3b14e224e516b165871fc60e58105`; origin integration still names that commit. Recovery head `4ba21a65b7e7f7abe79b5381f4ec20a4ef341c1d` preserves the independent BB-01/02/03 evidence. The approved RES-01 chain (`a4f66d2`, `0b87ae6`, `e095d72`), RES-04 chain (`9327b21`, `045076c`), and RES-02/03/05 chain (`3677bd8`, `6eb29dd`, `7a4d70f`) are integrated exactly once. Patch-equivalent parser/freshness bases already present in the candidate were not replayed. No push, merge to main, service restart, shared database use, paid call, native build, EAS action, or release action occurred.

RES-01 now transfers an accepted parent view only to the exact owned child after durable accepted identity and strict child refresh; same-run confirmation retains its parent lease. Lost GET, local persistence failure, cancellation, unrelated navigation, consent revoke, logout, deletion and account replacement remain fail-closed and restart from the last durable identity. RES-04 applies strict SHA-256/key/payload/phase parsing to follow-up, assumption and correction journals. Confirmation is bound to the parent run; corrupt or ambiguous records remain held and cannot authorize a POST or silent clear. The recovery AG07 skip is replaced by active sibling-parser behavior.

RES-02/03/05 uses question-bound `requested-obligations.v2` entity×fact pairs, authoritative assertion scope and production Evidence Needs/limited-publication disclosure. The first implementation (`3677bd8`) and first repair (`6eb29dd`) were independently rejected before the pair-binding repair (`7a4d70f`). New freshness writes use `criterion-freshness.v4`; stored v2 meaning remains immutable and valid v3/v4 rows must match their exact stored version and owner. Present workforce/`now` is current; fiscal, quarter, end-of-period and explicit `as of` controls remain historical. Recovery assertions were retained under the final semantics; their original exact blobs remain in `4ba21a6` ancestry.
Isolated worktree `/home/oranolio/Desktop/deep-v8-res02-20260920`, branch `codex/v8-res02-freshness`, base `3116f6e88074b51cd8f222e775ffcb2fdb8d4d9d`. **Not merged to main/integration and not pushed.** Independent review rejected `3677bd83e028ac6f52d162cf707cd2094793c7c7`, `6eb29dd28fce399c78bfa6d54ff6a2e6b089dc9d`, and `7a4d70fb9df8d98157fe78c587a91ccb4d8545d9`. The third rejection showed that capitalization-dependent subject extraction could still false-complete a lowercase compound request and that a self-consistent but semantically wrong v2 row could be restored. `requested-obligations.v2` now derives question-bound entity×fact obligations from grammar-bound subject spans independent of capitalization. Assertion scope is authoritative, every requested pair needs its own relevant supported assertion, and bare `expand` plus expanded/expansion variants share the expansion fact identity. Lowercase, case-mixed, comma, slash, multi-sentence and deeper-subquestion forms stay compound; an ambiguous plural subject fails closed; atomic single-fact controls still do not.
Isolated worktree `/home/oranolio/Desktop/deep-v8-res02-20260920`, branch `codex/v8-res02-freshness`, base `3116f6e88074b51cd8f222e775ffcb2fdb8d4d9d`. **Not merged to main/integration and not pushed.** Independent review rejected `3677bd83e028ac6f52d162cf707cd2094793c7c7`, `6eb29dd28fce399c78bfa6d54ff6a2e6b089dc9d`, `7a4d70fb9df8d98157fe78c587a91ccb4d8545d9`, and `2cc678fa689ce1a1d4688d137a278f82399abf83`. The fourth rejection found that case-mixed `ARDENT labs` produced a phantom fallback `ARDENT` entity, so all four correct entity×fact assertions could not complete. `requested-obligations.v2` now derives question-bound entity×fact obligations from grammar-bound subject spans independent of capitalization, and suppresses only strict token-subspan aliases contributed by the title-case fallback. Explicitly requested overlapping names such as `Alpha` and `Alpha Labs`, acronyms and single-word entities remain distinct. Assertion scope is authoritative, every requested pair needs its own relevant supported assertion, and bare `expand` plus expanded/expansion variants share the expansion fact identity. Lowercase, case-mixed, comma, slash, multi-sentence and deeper-subquestion forms stay compound; an ambiguous plural subject fails closed; atomic single-fact controls still do not.

New writes use `criterion-freshness.v4`; production restores insert-once v2/v3/v4 rows without overwriting them. A v2 row is valid only on the default key for the exact admitted original question and when its canonical semantics equal deterministic recomputation under the stored v2 algorithm. v3/v4 row columns, canonical JSON keys, owner and exact deterministic semantics under the stored version must also match or resume fails closed. The authoritative criterion policy is the first write for that key. Undated present workforce facts are current; fiscal-year-ended/ending, fiscal year, quarter, end/start and other fixed-period controls are historical, while explicit current/live wording remains current.

Repair evidence: full research-core **366/366**, focused semantic coverage/freshness **64/64**, research-core/backend/workspace typechecks, boundaries and governance pass. A fresh representative PostgreSQL run proves the case-mixed crossed-pair path remains limited and the full-four-pair path completes **2/2** (`deep_res02_alias_final_20260920_2016`). Earlier fresh isolated PostgreSQL evidence passes all historical siblings **8/8**, including the lowercase compound, wrong-scope mention, crossed entity×fact pairs, fiscal/current controls, atomic controls and valid v2 resume (`deep_res020305_final_lower_v2_20260920_1955`). A separate fresh database passes authoritative first-write plus valid/tampered v2/v3/v4 replay **2/2** (`deep_res020305_persistence_v2_20260920_1950`). Fabricated nonbillable transport only; no live/network provider call. Full `pnpm verify` and the full PostgreSQL suite are not claimed.

Integrated deterministic evidence is green: `pnpm verify` passes workspace types, research-core **392/392**, backend unit **247/247**, mobile **504/504**, boundaries and governance **6/6**. The mobile focused union passes **210/210** with no skip; mobile/backend typechecks pass. Prior independently reviewed isolated-PostgreSQL evidence remains **8/8** worker siblings plus **2/2** policy first-write/replay/tamper controls; those database tests were not rerun in this integration worktree. Exact commands, original blob identities and rollback are in `verification/v8/recovery-rec01-rec02/RESULTS.json`.

APPLE-01 is prior owner-authorized external readback, not an integration-lane execution claim. It records team `W6MVPS68ZU`, explicit App ID `app.deepresearch.mobile` with Sign in with Apple, and App Store Connect app `6814282574` (`Norrow: AI Deep Research`, version `1.0`, `PREPARE_FOR_SUBMISSION`) with HTTP-200 metadata readback. The source main worktree retains the portal screenshots outside this clean candidate; they were not copied or promoted into local application evidence. No signed build exists, the production EAS profile still targets `https://example.invalid`, and no TestFlight upload/submission/release is authorized or performed.

Main dirt, the dirty `codex/phase-a-state` checkout, 18 unique integration visual-QA files, source lanes and active ports 8787/8788/8790 remain untouched. The MC-D01 financial hold and all consent, ownership, deletion, publication and unknown-outcome gates remain unchanged.
## RES-04 durable journal parity — 2026-09-20

Isolated worktree `/home/oranolio/Desktop/deep-v8-res04-20260920`, branch `codex/v8-res04-sibling-journals`, base `b818a1c07354b78d30cd8ed88bf1d37a36dfa70f`, initial implementation `9327b21fc3b7b602f34aa8607ada9b8b7d9add97`, foreign-confirmation repair `045076cc181336bb581f060de3b7ca6d14f4b7e9`, terminal-binder repair `1b7640ac1b90454fe625620d585afb8de5cc9703`, state-gate repair `62a374c9dadbe503610eaf974c4e0833e024de90`. **Local only; not merged to `main` and not pushed.**

Follow-up, assumption and correction durable records now match the BB-03 fail-closed parser boundary: lowercase SHA-256, recomputed payload/key binding, immutable UUID request identity, exact known phase, phase-specific accepted fields, and valid accepted run UUID. Confirmation is same-run: an accepted/adopted confirmation naming any other well-formed UUID is damaged and held rather than adopted or cleared. Every extant record is parsed before terminal/unresolved branching. In particular, a damaged journal labeled `adopted`, `rejected` or `withdrawn` cannot be replaced with a new request identity or authorize save, POST, adoption, admission, New Research, routed follow-up, Android Back clearing, or silent clear. State gates parse all three non-null journals before checking any phase, preserve the raw in-memory record and current run on damage, and allow a deliberate new request from a valid terminal row only after strict validation. Current unversioned, fully bound records remain readable; legacy records missing identity are deliberately unsupported and held for device cleanup rather than implicitly migrated. Protected-cache damage remains intact while hydration fails.

The temporary BB-03 source-presence assertion is behavioral. RES-04 covers corruption matrices, accepted restart/no-second-POST recovery, same-run confirmation, foreign-confirm hold, storage-damage restart and all three terminal phases across all three journal types. The first independent review rejected foreign confirmation at `9327b21`; integrated review rejected evidence head `2abcbd1` because `bindPendingFollowUp` inspected terminal phase before parsing; the next review rejected evidence head `a7b6d52` because `canSubmit` and `startNewResearch` still used phase-only checks. Exact state-gate reproduction failed **9/19** before `62a374c`; repaired state gates pass **21/21**. Current verification: focused durable journals **151/151**, full mobile **468/468**, mobile typecheck EXIT 0, boundaries `ok`. No DB, network, provider, native, or paid call. This repair awaits another independent rereview; it is not self-approved. Rollback: revert `62a374c`, `1b7640a`, `045076c`, then `9327b21`, while retaining strict readers and held records; no schema or dependency rollback is needed.
