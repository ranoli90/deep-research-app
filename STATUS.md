...
W04 partial: Trafilatura 2.2.0 in a network-disabled offline process replaces controlled-fetch HTML prefixes. Saved bytes, transport/extraction receipts and block/table locators persist under ownership; HTML stays partial. Actual parser tests 3/3; storage/deletion test 1/1. Actual adapter retained 11/11 selected real-document spans; one additional oversized download failed. References were agent-inspected, not human-adjudicated. PDF/binary upload and broader extraction quality remain open.

W03 local deletion repair now purges current raw/derived stores under the account lock, stores new attachment bytes atomically, retries legacy disk cleanup durably, replays prior tombstones and rejects late API content writes. Nine PostgreSQL/filesystem regressions include real lock contention; hosted backup/retention and mobile cache behavior remain unverified.
