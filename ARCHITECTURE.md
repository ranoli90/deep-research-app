...
9. An explicit requested claim check captures immutable report/claim provenance and creates a separately admitted child. Its worker reuses selected passages or rereads selected known sources, then restores scoped support through the existing model gateway. It performs no discovery. Optional private feedback is stored but not assessed. Negative reports link contrary evidence; publication independently restores the target and result. Bound retry recovery, eight-child lineage limit, deletion and required-proof rollback rules are specified in ADR037 and ENGINE_CONTRACTS.

This is implemented composition, not proof of general research quality. Local API/worker/PDF and corrected arithmetic journeys use fabricated model transport. At historical checkpoint `e225988`, counterevidence/challenge execution was unfinished; it and explicit requested verification now have local implementation controls. General eligibility and semantic applicability, selective correction dependencies, larger evidence selection and independently evaluated held-out same-pipeline quality remain open. Preliminary source-only coverage can still cause unnecessary discovery for arithmetic questions. The two-run corrected arithmetic journey measured 31.860 seconds in the recorded full integration run; repeated proof restoration remains a latency concern.
...
The historical `research-controller.v1`, `selectAdaptiveAction`, `selectBaselineAction`, fixture catalog and scenario composer live behind the separate development-only diagnostic entrypoint. `LIVE_CONTROLLER_KIND=baseline` is a historical diagnostic selector, not a switch for the current production worker. `fixture` and `hosted-baseline` labels remain public/legacy contract vocabulary; they do not establish an implemented competent same-pipeline evaluation arm. Matched A1/B strategy code and local fabricated-transport controls use the same production pipeline (ADR026); the real-model, independently adjudicated baseline/candidate comparison required by `EVALUATION.md` remains unfinished. Do not promote hosted citation metadata to full-text evidence or infer live quality from deterministic diagnostic passes.
...
Corrections: migration024 owns immutable run-evidence memberships and change sets. Snapshot reuse references authorized source versions rather than copying evidence or inheriting approvals; full child support recomputation is the conservative policy. Exact accepted-question span edits now supplement whole-question replacement: contracts validate the strict patch, backend admission computes the owned original SHA256 under revision locks, and a pure core helper derives the new question with UTF-16/quote/Unicode checks. Existing change sets retain the entire accepted patch and correction text. This API granularity does not provide semantic criterion editing UI, selective dependency traversal or recomputation savings; full recomputation and conservative rediscovery remain. Real-model corrected-result equivalence remains unverified; ADR040 owns the input contract and rollback. Account deletion includes both original and derived records and takes precedence over report history.
...
Mobile ProfilePanel owns Settings presentation and accessible action wiring only; App retains account/session mutation authority. It imports public UI types, never server credentials or orchestration.

Append-document corrections reuse the existing admission/upload paths and immutable parent evidence; the worker separately requires readable owned new-document evidence before recomputation (ADR048). Mobile uses a dedicated protected correction journal, actual child snapshot adoption and account/view fences. Recovery never grants fresh processing authority.

The registered evaluation runner is an evaluator-only composition of the same API/worker/parser/tools/checker, excluded from production imports. Explicit current operator-attested authorization gates admission; durable journals retain source/config/attempt/cost identities and failures. It implements live discovery only, not frozen supplied-document execution or automatic scoring (ADR049). Remote source invalidation exposes the existing owned tombstone to protected mobile cleanup without adding a new persistence service (ADR050).

W08 evaluator composition (ADR052) supports registered frozen PDFs through the existing authenticated binary-upload/API/worker path, with identical extraction/support/writer/checker and mode-specific disabled discovery. The evaluator-only file loader/projection and private trace journal cannot enter production API/worker import graphs. Unsupported HTML is an unrun evaluation input, not a substitute extractor or production fallback. Migration037 only aligns retained document artifact capacity with existing8MiB upload admission (ADR051); public fetch and model-context limits are unchanged.

ADR054 extends the existing owned binary ingestion/evaluator projection to bounded saved UTF-8 HTML. The production isolated structure-v3 extractor is reused without new network capabilities or services. Supplied-file provenance is retained; this does not add HTML to the native picker or imply live retrieval. Legacy evaluator statements above that frozen HTML is unsupported describe the ADR052 checkpoint.

ADR055 adds a pure whole-passage retrieval policy in research-core and a backend-owned evidence selection proof module/table. It uses existing authorized_run_passages, model gateway, worker fencing, publication and deletion boundaries; no graph/vector service, new provider or client orchestration. Per-run policy preserves legacy in-flight request identities; new strict context metadata is pinned in model-input.v5. Omission proof is separate from substantive claim support and forces explicit partial-assessment limitations.
