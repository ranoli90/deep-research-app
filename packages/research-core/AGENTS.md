...
**Current status:** Pure controller policy (`research-controller.v1`), citation support checks, and publication fences live in `src/`. Adaptive selection is `selectAdaptiveAction`; the bounded chooser is `selectBaselineAction`. Query provenance/expansion, source-type planning, evidence-value breadth, freshness, reconciliation and origin clustering are additional pure policy (`query-provenance.v1` and related). No HTTP, React Native, or provider SDKs.
