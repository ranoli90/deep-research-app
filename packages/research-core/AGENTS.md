...
**Current status:** Pure controller policy (`research-controller.v1`), citation support checks, and publication fences live in `src/`. Adaptive selection is `selectAdaptiveAction`; the bounded chooser is `selectBaselineAction`. `compileResearchIntent` / `evaluateClarificationValue` (`research-intent-compiler.v2`, `RESEARCH_INTENT_CAPABILITY`) turn one-sentence questions into structured objectives without rewriting the original question: deterministic preflight plus a provenance-checked overlay. This is not a general semantic planner and has no model operation. Query provenance/expansion, source-type planning, evidence-value breadth, freshness, reconciliation and origin clustering are additional pure policy (`query-provenance.v2` and related). `explainFromExistingEvidence` (`explain-from-existing-evidence.v1`) answers post-report explain follow-ups from owned report text and supporting passage exact texts without mutating the brief or inventing citations. `planTypedQuery` must not copy private/source wording into public queries. Retrieved source text cannot escalate tools, budget, or consent. No HTTP, React Native, or provider SDKs.
...
A pure helper is not a shipped capability until the production worker calls it with real inputs. Do not treat unit-tested `nextSourceClass`, `evaluateFreshness`, `reconcileDocumentClaim`, or clustered independence as done if the worker still passes `sources:[]`, hardcodes `freshnessUnmet: false`, or never persists reconciliation. `explainFromExistingEvidence` is unfinished until `POST /v1/runs/:id/follow-up` loads the latest owned report and authorized passages and returns that answer.
...
Query provenance is fail-closed. Classify terms as user-public, safe-application-derived, public-evidence-derived, private-document-derived, or unclassified. Expand only from a bounded application lexicon and source-class qualifiers after the question-span binding gate. Never copy retrieved snippet text or private document wording into a public query. Canary substrings alone are insufficient: document-only tokens that are not canonical public terms require approval. Unknown leftover tokens are unclassified and blocked, never user-public. Approved private terms apply only to the query digest they were granted for.
